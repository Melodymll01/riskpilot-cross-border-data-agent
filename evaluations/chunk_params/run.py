...
TEST_CASES_BY_FILE = {
    "个人信息保护法": [
        ...
        {"question": "处理多大年龄以下未成年人的个人信息需要监护人同意？",
         "must_contain": "不满十四周岁"},
        # —— 权利题：撤回同意
        {"question": "个人能否撤回对个人信息处理的同意？",
         "must_contain": "便捷的撤回同意的方式"},
        # —— 权利题：可携带权
        ...
        # —— 概念题：敏感个人信息的同意要求
        {"question": "处理敏感个人信息在同意上有什么特殊要求？",
         "must_contain": "单独同意"},
    ],
    "数据出境安全申报": [
        {"question": "哪些情形需要申报数据出境安全评估？",
         "must_contain": "关键信息基础设施"},
        {"question": "数据出境安全评估申报需要提交哪些材料？",
         "must_contain": "申报书"},
        {"question": "数据出境风险自评估报告应当包含哪些内容？",
         "must_contain": "风险"},
        {"question": "国家网信办多久会出具评估结果？",
         "must_contain": "工作日"},
        {"question": "与境外接收方订立的法律文件应当明确什么事项？",
         "must_contain": "境外接收方"},
        {"question": "重要数据出境的申报要求是什么？",
         "must_contain": "重要数据"},
        {"question": "申报流程的受理机关是哪一级？",
         "must_contain": "省级"},
        {"question": "数据处理者如何准备数据出境的自评估？",
         "must_contain": "自评估"},
    ],
}
...
def find_default_file() -> str:
    ...
    files = [f for f in os.listdir(DEFAULT_UPLOAD_DIR)
...
def main():
    ...
    parser.add_argument("--files", type=str, nargs="+", default=None, help="指定多个测试文档路径（覆盖 --file）")
    parser.add_argument("--all", action="store_true", help="评测 uploads 目录下所有文件")
    parser.add_argument("--with-retrieval", action="store_true", help="同时评估检索命中率（会调用 embedding API）")
    ...
    for file_path in files_to_eval:
        ...
        for params in PARAM_GRID:
            ...
            print(f"  chunk 数: {quality['num_chunks']}  "
            ...
            if args.with_retrieval and test_cases:
                ...
                print(f"  命中率: {hit_result['hit_rate']:.0%} ({hit_result['hits']}/{hit_result['total']})")
        ...
        print(f"{'='*60}")
        header = f"{'参数':<12} {'chunk数':>7} {'avg':>5} {'min':>5} {'max':>5} {'过短':>4} {'过长':>4}"
        if args.with_retrieval:
            header += f" {'命中率':>7}"
        print(header)
        for r in results:
            row = (f"{r['chunk_size']}/{r['chunk_overlap']:<5} "
